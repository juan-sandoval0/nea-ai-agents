...
REST client for Harmonic.ai company intelligence API.
...
class HarmonicCompany:
    ...
    def from_api_response(cls, data: dict) -> "HarmonicCompany":
        ...
        return cls(
            ...
            funding_stage=funding.get("current_stage") or funding.get("currentStage"),
            funding_last_amount=funding.get("last_amount") or funding.get("lastAmount"),
            funding_last_date=funding.get("last_date") or funding.get("lastDate"),
...
class HarmonicClient:
    ...
    def _extract_company_id(self, entity_urn: str) -> Optional[str]:
        """Extract company ID from entity URN (e.g., 'urn:harmonic:company:142540' -> '142540')."""
    ...
    def lookup_company(
        ...
        Uses typeahead search to find the company, then fetches full details.
        ...
        # Use typeahead search to find the company
        ...
        try:
            ...
            # Find matching company from results
    ...
    def get_company(self, company_id: str) -> Optional[HarmonicCompany]:
        ...
        Args:
            company_id: Harmonic company ID
    ...
    def _extract_person_id(self, entity_urn: str) -> Optional[str]:
        """Extract person ID from entity URN (e.g., 'urn:harmonic:person:187397664' -> '187397664')."""
    ...
    def get_company_employees(
        ...
        Args:
            company_id: Harmonic company ID
            employee_type: Filter type - "all", "founders", "executives", "engineering"
            limit: Maximum results to return
            fetch_details: If True, fetch full person details (slower). If False, return URNs only.

        Returns:
            List of HarmonicPerson objects
        """
        params = {"size": limit}

        # Map employee type to API parameter
        type_map = {
            "founders": "FOUNDERS",
        ...
        try:
            ...
            for item in results[:limit]:
                # Results are URNs like "urn:harmonic:person:187397664"
                if isinstance(item, str):
                    ...
                    # If API returns full objects in future
    ...
    def search_typeahead(
        ...
        Returns partial data - use get_company() for full details.

        Args:
            query: Company name or domain prefix
            limit: Maximum results

        Returns:
            List of HarmonicCompany objects (partial data from typeahead)
        """
        params = {
            "query": query,
        }
            ...
            for result in results[:limit]:
                if result.get("type") == "COMPANY":
                    ...
                    if company_id and company_id not in seen_ids:
                        ...
                        # Create partial company from typeahead result
    ...
    def find_similar_companies(
        ...
        Args:
            company_id: Harmonic company ID to find similar companies for
            limit: Maximum results

        Returns:
            List of similar HarmonicCompany objects
        """
        params = {"size": limit}
    ...
    def lookup_person(self, linkedin_url: str) -> Optional[HarmonicPerson]:
        ...
        Returns:
            HarmonicPerson if found, None otherwise
        """
        try:
