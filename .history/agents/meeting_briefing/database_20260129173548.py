"""
Database Schema and Models for Meeting Briefing Agent
======================================================

SQLite-based storage for structured company data.
Tables mirror the Google Sheet schema:
- company_core
- founders
- news
- key_signals
...
Usage:
    from agents.meeting_briefing.database import Database, CompanyCore, Founder
...
DEFAULT_DB_PATH = Path(__file__).parent / "meeting_briefing.db"
...
class Database:
    """
    SQLite database for meeting briefing structured data.
    ...
    def upsert_company(self, company: CompanyCore) -> None:
        ...
        Args:
            company: CompanyCore object to upsert
        """
        conn = self._get_connection()
        try:
            cursor = conn.cursor()
    ...
    def get_company(self, company_id: str) -> Optional[CompanyCore]:
        ...
        Args:
            company_id: Company identifier (URL/domain)

        Returns:
            CompanyCore object or None if not found
        """
        conn = self._get_connection()
        try:
            cursor = conn.cursor()
    ...
    def upsert_signals(self, signals: list[KeySignal]) -> None:
        ...
        Replaces existing signals of the same type.

        Args:
            signals: List of KeySignal objects
        """
        if not signals:
            return
    ...
    def get_company_bundle(self, company_id: str) -> CompanyBundle:
        """
        Get complete company data bundle for briefing generation.

        This is the read-only accessor used by the briefing generator.

        Args:
            company_id: Company identifier

        Returns:
            CompanyBundle with all available data
        """
